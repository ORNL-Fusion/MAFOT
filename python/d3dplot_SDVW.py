...
def d3dplot(pathname, printme = False, coordinates = 'psi', what = 'psimin', machine = 'd3d', 
            ...
            reverse_y = False,manifolds = None, correctPFR = True, Lcmin = None, points_file = None,
            manifold_color=None):
    """
    plot MAFOT results
    --- user input ------------------
    ...
    if(coordinates == 'RZ'): # plot wall
        ...
        if manifolds is not None:
            ...
            for item in manifolds:
                try:
                    if manifold_color == "col":
                        xy, st = readfile_mani_with_state(item)
                        plot_manifold_with_state(ax, xy, st, lw=1.0, zorder=6)
                    elif manifold_color == "suffix":
                        mandata = readfile(item)
                        c = manifold_color_from_name(item)
                        ax.plot(mandata[:, 0], mandata[:, 1], "-", color=c, linewidth=1, zorder=6)
                    else:
                        # original behavior
                        mandata = readfile(item)
                        ax.plot(mandata[:, 0], mandata[:, 1], "k-", linewidth=1, zorder=6)
                except Exception as e:
                    print(f"WARNING: could not plot manifold '{item}': {e}")
    ...
    # --- overlay labeled points (h_i) ---
    if points_file is not None:
        try:
            ptab = read_points_table(points_file)
            xp, yp = points_xy_for_coordinates(ptab, coordinates)
    
            if xp is None or yp is None:
                print(f"WARNING: points_file provided, but cannot map it to coordinates='{coordinates}' (no phi in file). Skipping.")
            else:
                ax = plt.gca()
    
                # points
                ax.scatter(xp, yp, s=5, c='red', marker='o', zorder=10)
    
                # labels (h_i) with auto offset direction
                x0, x1 = ax.get_xlim()
                y0, y1 = ax.get_ylim()
                ...
                for i, (xi, yi) in enumerate(zip(xp, yp), start=1):
                    lbl = rf"$h_{{{i}}}$"
                    ax.text(
                        xi + dx_data, yi + dy_data,
                        lbl,
                        fontsize=8,
                        ha="left",
                        va="bottom",
            ...
            print(f"WARNING: could not plot points from '{points_file}': {e}")
    ...
    if not (typeOfPlot in ['poincare','manifold']):
        ...
        if(coordinates == 'RZ') | (coordinates == 'phi'): 
            myticklabels = [format(item,'g') for item in myticks]
...
def read_points_table(fname):
    """
    Read a 'fixed point' style file:
    - arbitrary header lines starting with '#'
    - numeric table with at least 6 columns:
      R  Z  period  psi  theta  r

    Returns a (N, M) float array.
    """
    tab = readfile(fname)  # your existing fast reader: ignores lines containing '#'
    if tab.ndim == 1:
        tab = tab[np.newaxis, :]
    if tab.shape[1] < 6:
        raise RuntimeError(
            f"Points file '{fname}' must have at least 6 columns: R Z period psi theta r"
        )
    return tab


def points_xy_for_coordinates(tab, coordinates):
    """
    Given the points table (N,>=6), return (xp, yp) in the CURRENT plot coordinates.
    """
    ...
    if coordinates in ('psi', 'pest'):
        # pest would ideally use theta_p from sfc.ev(theta, psi) which is not available here.
        # So we fall back to plotting (theta, psi) which is still meaningful.
        return theta, psi

    if coordinates == 'rtheta':
        return theta, r

    # anything involving phi can't be plotted from this file
    if coordinates in ('phi', 'phitheta', 'phiZ'):
        return None, None

    # unknown / unsupported
    return None, None

def manifold_color_from_name(fname):
    """Filename-based manifold coloring: _un -> red, _st -> green, else black."""
...
def readfile_mani_with_state(file):
    """
    Read a manifold file where the *last column* is a state tag: 'u' or 's'.
    Returns:
      xy : (N,2) float array with columns [x,y] (we use first two cols)
      st : (N,) array of strings, each 'u' or 's'
    Header/comment lines containing '#' are ignored (same behavior as readfile()).
    """
    xs, ys, st = [], [], []
    with open(file, "r") as f:
        for line in f:
            ...
            if len(parts) < 3:
                continue
            # first two columns are x,y (for manifold overlays this is R,Z in your usage)
            xs.append(float(parts[0]))
            ys.append(float(parts[1]))
            st.append(parts[-1].strip().lower())
    xy = np.column_stack([xs, ys]).astype(np.float64)
    st = np.array(st, dtype=object)
    return xy, st


def plot_manifold_with_state(ax, xy, st, lw=1.0, zorder=5):
    """
    Plot manifold polyline colored by state per row:
      'u' -> red, 's' -> green.
    Splits into segments whenever st changes.
    """
    if xy.shape[0] == 0:
        return

    # sanitize state labels to only 'u' or 's' where possible
    st2 = np.array(["u" if x.startswith("u") else ("s" if x.startswith("s") else "") for x in st], dtype=object)

    # indices where state changes
    change = np.where(st2[1:] != st2[:-1])[0] + 1
    breaks = np.concatenate(([0], change, [len(st2)]))

    for a, b in zip(breaks[:-1], breaks[1:]):
        seg = xy[a:b]
        if seg.shape[0] < 2:
            continue
        c = "r" if st2[a] == "u" else ("g" if st2[a] == "s" else "k")
        ax.plot(seg[:, 0], seg[:, 1], "-", color=c, lw=lw, zorder=zorder)

def collect_manifold_files_from_dir(d):
    """
    Return sorted list of files matching man_* in directory d.
    Sorted for stable ordering.
    """
    try:
        names = sorted(
            f for f in os.listdir(d)
            if f.startswith("man_") and os.path.isfile(os.path.join(d, f))
        )
        return [os.path.join(d, n) for n in names]
    except Exception as e:
        print(f"WARNING: could not scan directory '{d}' for man_* files: {e}")
        return []

def plot_overlays_only(
    *,
    coordinates,
    machine,
    toroidal_angle=0,
    ...
    """
    Make a blank figure and plot only:
      - wall (RZ canvas)
      - optional manifolds (-M / -A)
      - optional labeled fixed points (-s)
    """

    if coordinates != "RZ":
        print("WARNING: overlays-only mode is intended for -c RZ. Forcing coordinates='RZ'.")
        coordinates = "RZ"

    # --- layout defaults similar to your RZ branch ---
    width = 9 if figwidth is None else figwidth
    height = 7 if figheight is None else figheight
    ...
    # --- wall ---
    wall = get_wall(machine, toroidal_angle, wall3Dfile)
    if wall is not None and np.size(wall) > 0 and wall[0, 0] is not None:
        ax.plot(wall[:, 0], wall[:, 1], "k--", linewidth=2, zorder=3)
        ax.set_aspect("equal")

        # set default limits from wall unless user overrides
        extra = 0.05 * np.nanmin(wall[:, 0]) if np.isfinite(wall[:, 0]).any() else 0.0
        ax.set_xlim(np.nanmin(wall[:, 0]) - extra, np.nanmax(wall[:, 0]) + extra)
        ax.set_ylim(np.nanmin(wall[:, 1]) - extra, np.nanmax(wall[:, 1]) + extra)
    else:
        # no wall found; use generic limits unless user sets them
        ax.set_aspect("equal")
        ax.set_xlim(-2, 2)
        ax.set_ylim(-2, 2)

    # --- manifolds (same logic as in d3dplot overlay block) ---
    if manifolds is not None:
        for item in manifolds:
            try:
                if manifold_color == "col":
    ...
    # --- fixed points (reuse existing helpers) ---
    # --- overlay labeled points (h_i) ---
    if points_file is not None:
        try:
            ptab = read_points_table(points_file)
            ...
            if xp is None or yp is None:
                ...
                # points
                ax.scatter(xp, yp, s=5, c='k', marker='o', zorder=10)
...
if __name__ == '__main__':
    ...
    opts, args = getopt.gnu_getopt(sys.argv[1:], "hc:w:m:pg:t:P:N:C:T:ifW:H:Ub:x:y:L:lRM:F:s:A", ["help", "coord=", "what=", 
                                                                 "machine=", "printme", "graphic=", 
                                                                 "tag=", "physical=", "cmap=", "Title=", "imshow", "contourf",    
                                                                 "figwidth=", "figheight=", "unicode",
                                                                 "range=", "xlim=", "ylim=", "wall=", "log",
                                                                 "Reverse", "Manifold=", "pfr=", "points=",
                                                                 "manicolor", "manicolor=", "all", "all="])
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            print ("  -s, --points <Arg>  Fixed points file to plot and label hyperbolic points")
            print ("  -A, --all <Arg>  Auto loop through ./ (default) or a provided directory --all=./...")
            ...
            mode = (a or "").strip().lower()
            if mode == "":
                manifold_color = "suffix"   # no-arg form: filename-based (_un/_st)
            elif mode == "col":
                manifold_color = "col"      # arg form: per-row last-column u/s
            else:
                manifold_color = "suffix"   # any other arg -> fallback
        elif o in ("-A", "--all"):
            use_all = True
            # GNU getopt: -A may come with None (no-arg), or a string (dir)
    ...
    # --- expand -A/--all into manifolds list (compatible with -M) ---
    if use_all:
        if all_dir is None:
            # default: directory where the program was called from
            d = os.getcwd()
        else:
            d = os.path.expanduser(all_dir)
    
        auto = collect_manifold_files_from_dir(d)
    
        if manifolds is None:
            manifolds = auto
        else:
            # combine with -M, de-dup preserving order
            combined = list(manifolds) + list(auto)
            seen = set()
            out = []
    ...
    # --- decide whether we have a base pathname ---
    pathname = args[0] if len(args) > 0 else None
    
    if pathname is None:
        # overlays-only mode (points/manifolds/wall)
        plot_overlays_only(
            coordinates=coordinates,
            machine=machine,
            toroidal_angle=angle,
            wall3Dfile=wall3Dfile,
            reverse_y=reverse_y,
            xlimit=xlim,
            ylimit=ylim,
            figwidth=figwidth,
            ...
            manifold_color=manifold_color,
        )
    else:
        # normal mode (base file + optional overlays)
        d3dplot(
            pathname, printme=printme, coordinates=coordinates, what=what, machine=machine,
            ...
            reverse_y=reverse_y, manifolds=manifolds, correctPFR=cPFR, Lcmin=Lcmin, points_file=points_file,
            manifold_color=manifold_color
        )
