// Header-File for the ITER Drift Programs 
...
out << "# Target (0=vertical, 1=45°, 2=horizontal, 3=shelf): " << which_target_plate << endl;
...
if(mpi_rank < 1) cout << "ITER-coil (0 = off, 1 = on): " << useIcoil << endl << endl;
ofs2 << "ITER-coil (0 = off, 1 = on): " << useIcoil << endl;
...
// Set I-coil geometry
if(useIcoil==1) iterigeom_(&kuse[0][0],&nbands,&nloops[0],&nsegs[0][0],&xs[0][0][0][0],&dvs[0][0][0][0],&curntw[0][0]);
...
double R1,Z1;	// linear part left Corner
double R2,Z2;	// linear part right Corner
Array<double,1> R,Z,S;	// Curve
index = get_target();	// Get target plate corner positions
switch(which_target_plate)
{
...
p1(1) = R1 - EQD.RmAxis;	p1(2) = Z1 - EQD.ZmAxis;
p2(1) = R2 - EQD.RmAxis;	p2(2) = Z2 - EQD.ZmAxis;
...
else	// t < 0
{
	...
	p1(1) = R(index) - EQD.RmAxis;		p1(2) = Z(index) - EQD.ZmAxis;
	p2(1) = R(index+1) - EQD.RmAxis;	p2(2) = Z(index+1) - EQD.ZmAxis;
	d = p2 - p1;	
...
if(which_target_plate == 2) t *= -1;	// undo reverse t
