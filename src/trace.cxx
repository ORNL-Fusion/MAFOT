// Program traces field lines/orbist from start to wall for particle-drift with time dependent perturbations
...
#define USE_MPI
#if defined(ITER)
	#define program_name "itertrace"
#elif defined(NSTX)
	#define program_name "nstxtrace"
#elif defined(MAST)
...
int c;
opterr = 0;
while ((c = getopt(argc, argv, "hX:V:S:I:E:T:P:")) != -1)
switch (c)
{
case 'h':
	if(mpi_rank < 1)
	{
		cout << "usage: mpirun -n <cores> dttrace [-h] [-E ErProfile] [-I island] [-P points] [-S siesta] [-T Tprofile] [-V wout] [-X xpand] file [tag]" << endl << endl;
		cout << "Trace from start to wall." << endl << endl;
		...
		cout << "  mpirun -n 4 dttrace _trace.dat blabla" << endl;
...
// Output
LA_STRING filenameout = "trace" + praefix + ".dat";
...
int N_variables = 11;	// Number of output variables
...
if(mpi_rank < 1)
{
	...
	var[0] = "R[m]"; var[1] = "Z[m]"; var[2] = "phi[deg]"; var[3] = "psi"; var[4] = "Lc[km]"; var[5] = "psimin"; var[6] = "ntor"; var[7] = "Ri[m]"; var[8] = "Zi[m]"; var[9] = "phii[deg]"; var[10] = "psii";
	PAR.writeiodata(out,bndy,var);

	// restart run, if necessary
	// ***************************** WARNING!!! **********************************
	// writing out the results is random in this tool, but a restart requires writing out in order
	// the restart will run, but not work correctly for now!!!
	int donePackages = 0;
	...
	#pragma omp parallel shared(N_values,sent_packages,recieve_packages) private(i,tag,N_slave_use) num_threads(2)
	{
		...
		#pragma omp section	//-------- Master Thread: controlles comunication ----------------------------------------------------------------------------------------------------------------------
		{
			...
			while(workingNodes > 0)	// workingNodes > 0: Slave still working -> MPI:Revc needed		workingNodes == 0: all Slaves recieved termination signal
			{
				...
				#pragma omp critical
				{
					...
					{
						if(fabs(recieve(7,j) - PAR.itt) < 0.5) continue;	// don't write the points that are still inside
						out << recieve(1,j);
		...
		#pragma omp section	//------- Slave Thread on Master Node: does same calculations as Salve Nodes ----------------------------------------------------------------------------------
		{
			...
			while(1)
			{
				...
				Nmax_slave = N_values(tag*N_slave_use);
				...
				for(n=Nmin_slave;n<=Nmax_slave;n++)
				{
					...
					if(use_inputPointsFile)
					{
						FLT.R = inputPoints(n, 1);
						FLT.phi = inputPoints(n, 2);
						FLT.Z = inputPoints(n, 3);
						FLT.get_psi(FLT.R,FLT.Z,FLT.psi);
						FLT.theta = polar_phi(FLT.R-EQD.RmAxis, FLT.Z-EQD.ZmAxis);
					}

					// store initial values
					results(8,n-Nmin_slave+1) = FLT.R;
					results(9,n-Nmin_slave+1) = FLT.Z;
					results(10,n-Nmin_slave+1) = FLT.phi;
					results(11,n-Nmin_slave+1) = FLT.psi;

					// map the orbit to the wall intersection or itt turns
					ntor = FLT.intersect(PAR.itt, PAR.MapDirection);

					// Store results
					results(1,n-Nmin_slave+1) = FLT.R;
					results(2,n-Nmin_slave+1) = FLT.Z;
					results(3,n-Nmin_slave+1) = modulo2pi(FLT.phi/rTOd)*rTOd;
					results(4,n-Nmin_slave+1) = FLT.psi;
					results(5,n-Nmin_slave+1) = FLT.Lc/1000.0;
					results(6,n-Nmin_slave+1) = FLT.psimin;
					results(7,n-Nmin_slave+1) = ntor;
				} // end for n
				...
				#pragma omp critical
				{
					for(j=1;j<=N_slave_use;j++)
					{
						if(fabs(results(7,j) - PAR.itt) < 0.5) continue;	// don't write the points that are still inside
						out << results(1,j);
