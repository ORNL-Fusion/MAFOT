...
c
c	subroutine NSTXECGEOM
c
c
c=======================================================================
 
      subroutine NSTXECGEOM (kuse,nbands,nloops,nsegs,xs,dvs,curntw)
 
c=======================================================================
c                                      Started by: DM Orlov, 2009 jun 12
c                                      Last Modified: A. Wingen, 2011 mar 17
c-----------------------------------------------------------------------
c NSTXECGEOM reads geometry-defining information, then writes arrays of
c NSTX EC-coil geometry parameters in the form required by DMO's 
c routines NSTXECCOILS, POLYGONB, BIOTLOOP.
c
...
c
c Even though the NSTX external Error Correction coil geometries are 
...
c  maintain.
c
c  Maximum array dimensions set by INCLUDE NSTXECCOILS.i:
...
c
c INPUTS:
c  NSTXECGEOM gets parameters from the NAMELIST input file nstx.in.
...
c-----------------------------------------------------------------------
      ...
      INCLUDE 'nstxeccoils.i'	! gives mxECbands, mxECloops, mxECsegs,
				! ncturns (integer)
...
c      INCLUDE 'constsi.i'
!        Generated by redefine_com.pro  20080512 tbt
      common /consts/  pi, twopi, cir, rtd, dtr
      common /currents/ ECadj, ECcur 
...
!  Local and Namelist Variables:
      real*8  dp0, dp1
      logical useNSTXECcoil, livecoil
...
! Set the FIXED geometry parameters of the 1 band of NSTX external
! Error Correction Coils:
 ! Using a fortran90 array initialization method
 ! This is a simple 6-sided model to roughly approximate NSTX EC coils
 
 ! ECloops(1:L) = number of coils (loops) in band (row) L  
      ECloops(1:1) = (/6/) 	! 1 band (mid) of 6 coils

 ! ECgeom(1:7,L)= R1,Z1,R2,Z2, phicentr1, phispan, dphi of band L (m,deg)
 ! This is a very (!!!) approximate representation of NSTX EC coils
...
c      CALL NSTXECREAD (useNSTXECcoil, ECadj, ECcur)
c changed by Wingen: ECadj and ECcur are read elsewhere and included here 
c					 through common block
 
 
c      if(.not. useNSTXECcoil)   RETURN	! don't need NSTX EC-coils
...
c      write(81,*) nbands,' NSTX EC-coil bands'
...
c  Cartesian component, segment, loop or coil, band or row.  
      ...
      DO  L=1, nbands  		! do all bands
       ...
       dphi      = ECgeom(7,L)
       addangl   = ECadj(1,L)
       arcmax    = abs(ECadj(2,L))	! must be .gt. 0

       phianchor = phianchor + addangl
       
       IF(arcmax .eq. 0. .and. ECadj(3,L) .ne. 0.) THEN
        write(*,*)'NSTXECGEOM: ECadj(2,*) = arcmax = 0 not allowed.'
	  write(*,*)'NSTXECGEOM: Stopping.'
	  STOP
       END IF
...
c     set sufficient number of toroidal segments for coils in band(L) 
	 ...
	 IF(ntorsegs .gt. 100) THEN		! stop runaway loop
	  write(*,*)'NSTXECGEOM: Coil segment arc size setup ran away.'
	  write(*,*)'NSTXECGEOM: Stopping.'
	  STOP
	 END IF
       END DO				! end of 'WHILE'
...
c      write(93,'(I3,A)'),nbands,'   bands. EC'
c      DO  L=1,nbands
...
c NSTXECGEOM become unavailable for execution by following procedures 
...
C      WRITE(*,*) 'LEAVING NSTXECGEOM'		! for DEBUG
 
      RETURN
      
 
      END 	SUBROUTINE	NSTXECGEOM
c=======================================================================
