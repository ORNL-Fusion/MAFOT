function [Z,X,Y,Nt,Np] = iterfoot_function(FileToOpen,phase,Target)
%clear;
%---------------- Set parameter here -------------------------------
% Color scaling in b
...
% (WhatShallIPlot = 2 only)  Comment, if not needed;
Limit = -5;    %-8.0;  % onset of private flux region, sets psmin=1 in pfr
...
if(printme==1)
    ...
    if(phase<10), filenameout = [FileToOpen(1:length(FileToOpen)-14) praefix];
    elseif(phase<100), filenameout = [FileToOpen(1:length(FileToOpen)-15) praefix];
    else filenameout = [FileToOpen(1:length(FileToOpen)-16) praefix];
    end
...
if(spare_interior == 1 && Laminar == 1), Z(Z == 4) = 0; end

% --- Set psimin = 1 in private flux region -----------------------
if(WhatShallIPlot == 2 && exist('Limit','var')==1) 
    if(Target==2), Z(Y <= Limit) = 1; 
    else Z(Y >= Limit) = 1; 
    end
end
...
if(Laminar==1)
    wallfile=fopen('/Users/wingen/c++/iter/wall.dat');
...
if (Laminar==1)
    ...
    ylabel({'t [cm]'},'FontName',schrift,'FontSize',schrift_size);
    if(Grid==1), grid on; end
end
if(phase<10), text(0.85,-0.12,['Phase: 00' num2str(phase) '-0-' num2str(114-phase)],'FontName',schrift,'FontSize',schrift_size,'Units','normalized');
elseif(phase<100), text(0.85,-0.12,['Phase: 0' num2str(phase) '-0-' num2str(114-phase)],'FontName',schrift,'FontSize',schrift_size,'Units','normalized');
else text(0.85,-0.12,['Phase: ' num2str(phase) '-0-' num2str(114-phase)],'FontName',schrift,'FontSize',schrift_size,'Units','normalized');
end
