...
// boundary
// assume dphi/dx = 0
// 0.5*(pki(k,j+1,i) - phi(k,j-1,i))/dZ = 0   -->   pki(k,j+1,i) = phi(k,j-1,i)  at Z boundary
// 0.5*(pki(k,j,i+1) - phi(k,j,i-1))/dZ = 0   -->   pki(k,j,i+1) = phi(k,j,i-1)  at R boundary
for(k=1;k<=DIVB.Np;k++)
{
	j = 1;
	for(i=2;i<=DIVB.NR-2;i++)
	...
	{
		R_half = 0.5*(DIVB.R(i+1) + DIVB.R(i));
		phi1p = 0.25*(phi(k,j-1,i) + 2*phi(k,j,i+1) + phi(k,j-1,i+1));
		phi1m = 0.25*(phi(k,j-1,i) + 2*phi(k,j,i-1) + phi(k,j-1,i-1));
		phi2p = 0.25*(phi(k,j-1,i+1) + 2*phi(k,j-1,i) + phi(k,j-1,i-1));
		phi2m = 0.25*(phi(k,j-1,i+1) + 2*phi(k,j-1,i) + phi(k,j-1,i-1));
		phi1 = 0.25*(phi(k,j-1,i) + 2*phi(k,j,i) + phi(k,j-1,i));
		phi2 = 0.25*(phi(k,j,i+1) + 2*phi(k,j,i) + phi(k,j,i-1));
