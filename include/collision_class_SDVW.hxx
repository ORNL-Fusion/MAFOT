...
bool COLLISION::occurs(double Lc, const double x, double & meanfreepath, double & probability) 
{
	...
	if(pdf_model == PDF_GAUSSIAN) {
		// original erf-based cumulative probability
		prob = 0.5 + 0.5 * erf((l - mfp)/(stdev * sq2)); // sqrt(2) calculated in constructor for efficiency
	} else {
		// Poisson-like model: probability of at least one collision over distance l when mean free path = mfp
		// using exponential survival: P = 1 - exp(-l / mfp)
		if(mfp > 0) prob = 1.0 - exp(- l / mfp);
		else prob = 0.0;
	}
	probability = prob;
	// draw a new random number for this check
	rnum = ran0(seed);
