...
// a(lles) n(�tzliche) d(irekt) i(mplementiert)
...
// Z�hlen, wieviele Zeilen mit '#' beginnen, diese werden �bersprungen
while(1)
{
	in >> line;
...
in.open(name);	// Erneutes �ffnen der Datei
for(i=1;i<=count;i++)	// �berspringen der IO-Daten Zeilen
{
	in >> line;
}

// Einlesen der Daten 
while(in.eof()==0) // Die letzte Zeile wird immer doppelt eingelesen --- leider nicht zu �ndern ---
{
	in >> parname;
	in >> wert;
	vec.push_back(wert);
}
// L�schen der letzten Eintr�ge, um doppeltes Einlesen zu kompensieren
vec.erase(vec.end()-1);
...
// Einlesen der Daten (hier f�r zwei Spalten)
while(in.eof()==0) // Die letzte Zeile wird immer doppelt eingelesen --- leider nicht zu �ndern ---
{
	in >> theta;
	in >> psi;
...
// Einlesen der Daten (hier f�r drei Spalten)
while(in.eof()==0) // Die letzte Zeile wird immer doppelt eingelesen --- leider nicht zu �ndern ---
{
	in >> A;
	in >> B;
...
// Einlesen der Daten (hier f�r vier Spalten)
while(in.eof()==0) // Die letzte Zeile wird immer doppelt eingelesen --- leider nicht zu �ndern ---
{
	in >> A;
	in >> B;
...
// Einlesen der Daten (hier f�r f�nf Spalten)
while(in.eof()==0) // Die letzte Zeile wird immer doppelt eingelesen --- leider nicht zu �ndern ---
{
	in >> A;
	in >> B;
...
// ------------- outputtest mit R�ckgabewert -----------------------------------------------------------------------------------
// see above
//int outputtest2(char* name)	// 0: File kann geschrieben werden	1: File existiert und soll nicht �berschrieben werden! 
//{
//int chk;
//string frage;
//ifstream in(name);
