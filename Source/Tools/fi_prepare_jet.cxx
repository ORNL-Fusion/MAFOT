...
// slices of current filaments for D3D-Drift with current filament perturbations
// A.Wingen						23.2.09
// M.Rack						20.05.2011
//	modify:	in "readiodata", read one more character for the EQD.Time.
//	add:	openMP to "prepare_filament_field"
...
void prepare_filament_field(LA_STRING filename);
double filament_fields_ex(double x, double y, double z, Array<double,2>& data, 
    int N, double I, double dIdt, double& B_x, double& B_y, double& B_z, 
    double& dAdt, double cosp, double sinp);
...
int main(int argc, char *argv[])
{
  ...
  prepare_filament_field(filename);
...
void readiodata(char* name, vector<double>& vec)
{
  LA_STRING input;	// !!! LA_STRING reads entire line, string reads only 
			// one word !!!
  ...
  in >> input;
  EQD.Shot = input.mid(9,6);	// 6 characters starting at index 9 of input 
				//string
  EQD.Time = input.mid(22,5);	// 5 characters starting at index 22 of input
				// string
  ...
  if(vec.size()>=23)
    ...
    cout << "No current filaments included" << endl;
}

//------------------ prepare_filament_field ------------------------------------
// Reads filament files and calculates total field on EFIT grid
// phi is discretized in 1 deg. steps, according to integrator step size  
// -->  no interpolation in phi!!!!!
// Bicubic interpolation on R and Z.
void prepare_filament_field(LA_STRING filename)
{
  // Variables
  int i,j,k;
  Array<double,4> field(Range(1,3), Range(0,359), Range(0,EQD.NR+1), 
			Range(0,EQD.NZ+1));
  ofstream out;
...
//    ofs2 << "Filament currents: ";
  ...
  for (i=1; i<=useFilament; i++) {
    ...
    if (data.rows() >= filament_data.cols()) 
      filament_data.resizeAndPreserve(useFilament,data.rows()+1,5);
    for(int j=1; j<=data.rows(); j++) 
      for(int k=1; k<=5; k++) 
	filament_data(i,j,k) = data(j,k);
      
    // number of rows in data is stored at col-index 0 and depth-index 1
    filament_data(i,0,1) = data.rows();
    
    // data no longer used; prepare data for next use
    data.free();

    // read current from filament file -> store it in filament_data(i,0,2)
    in.open(filamentfile);
    // location in the file needed !!!!!!!!!
    in >> line; in >> line; in >> word; in >> word;
...
//    ofs2 << i << ": " << filament_data(i,0,2) << "A" << "\t";
  ...
  #pragma omp parallel
  {
	...
	for (j=0; j<=EQD.NZ+1; j++) {
	  ...
	  // no filament fields, if useFilament==0
	  for (int l=1; l<=useFilament; l++) {
	    bx = 0; by = 0; bz = 0;
	    I = filament_data(l,0,2);	// current is stored in here, see 
					  // prep_perturbation
	    // filament_data(l,0,1) gives number of rows of data; index 0 is not 
	    // used in filament_fields_ex
	    slice.reference(filament_data(l,all,all));
	    dummy = filament_fields_ex(X,Y,Z,slice,int(filament_data(l,0,1)),I,
				       0,bx,by,bz,dummy,cosp,sinp);
	    // dIdt is set to 0 and fixed <- constant current dummy is used 
	    // instead of A_phi and dAdt
	    B_X += bx;
	    B_Y += by;
	    B_Z += bz;
	  }
  ...
  for (k=0; k<360; k++)
    for (i=0; i<=EQD.NR+1; i++)
      for (j=0; j<=EQD.NZ+1; j++)
	out << field(1,k,i,j) << "\t" << field(2,k,i,j) << "\t" <<
	    field(3,k,i,j) << endl;
...
//------------------ filament_fields_ex ----------------------------------------
// returns toroidal component A_phi of vector potential at position (x, y, z) at
// time t. data = data(1..N, 1..5) contains the pathway of the filament with 
// columns X, Y, Z, R, phi
// Current I(t) in [A] and its time derivative dIdt(t) has to be provided. t is 
// not needed!
// data usually has the form (R, Z, phi, psi) after reading from file -> has to 
// be converted in main program before calling this function.
...
// sinp = sin(phi) and cosp = cos(phi); phi is the toroidal angle of position, 
// phi is not needed
double filament_fields_ex(double x, double y, double z, Array<double,2>& data, 
    int N, double I, double dIdt, double& B_x, double& B_y, double& B_z, 
    double& dAdt, double cosp, double sinp)
{
  ...
  double B_x_tmp,B_y_tmp,B_z_tmp;
  double f;

  B_x_tmp = 0; B_y_tmp = 0; B_z_tmp = 0;
  A_x = 0; A_y = 0;
  ...
  for (i=1; i<N; i++) {
    ...
    // Help variables can get 0 <- not allowed --> Use another 
    // antiderivative (differs only on a constant -> same A)
    if (d1 <= 0 || d2 <= 0) {
      d1 = dist + (jx*dx1 + jy*dy1 + jz*dz1);
      d2 = dist2 + (jx*dx2 + jy*dy2 + jz*dz2);
      vor = -1;
    ...
    B_x_tmp += jz*dAy - jy*dAz;
    B_y_tmp += jx*dAz - jz*dAx;
    B_z_tmp += jy*dAx - jx*dAy;
  }
